...
export default function BlogCard({ post, isFeatured = false }: BlogCardProps) {
  return (
    <Link
      href={`/blog/${post.slug}`}
      className={`group bg-white border p-5 hover:border-black transition-all block ${
        isFeatured ? 'border-2 border-black' : 'border border-gray-200'
      }`}
    >
      ...
      {/* Заголовок */}
      <h3 className={`font-display font-bold text-black mb-2 group-hover:underline tracking-tight ${

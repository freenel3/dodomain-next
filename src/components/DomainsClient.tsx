...
export default function DomainsClient({ domains: allDomains }: { domains: Domain[] }) {
  ...
  useEffect(() => {
    ...
    if (extension !== null) setSelectedExtension(extension);
  }, [searchParams]);
  ...
  const extensions = useMemo(() => {
    ...
    // Safety check for null extensions
    const unique = Array.from(new Set(allDomains.map(d => d.extension || '').filter(Boolean)));
  ...
  const filteredDomains = useMemo(() => {
    return allDomains.filter(domain => {
      ...
      // Length filter (name without ext)
  ...
  const handleFilterChange = (setter: (value: string) => void, key: string) => (value: string) => {
    setter(value);
    setCurrentPage(1);
    updateUrl(key, value);
  };
      ...
      <div className="grid grid-cols-5 gap-3 mb-6">
        <div>
          ...
          <CustomSelect
            ...
            onChange={(value) => handleFilterChange(setSelectedExtension, 'extension')(value)}
            options={[

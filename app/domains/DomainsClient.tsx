...
export default function DomainsClient({ initialDomains }: DomainsClientProps) {
  ...
  useEffect(() => {
    ...
    if (extension) setSelectedExtension(extension)
  }, [searchParams])
  ...
  const extensions = useMemo(() => {
    ...
    const unique = Array.from(new Set(initialDomains.map(d => d.extension)))
    return customOrder.filter(ext => unique.includes(ext))
  }, [initialDomains])

  // Filter domains
  const filteredDomains = useMemo(() => {
    return initialDomains.filter(domain => {
      // Search filter
  ...
  }, [initialDomains, searchQuery, selectedExtension, priceFrom, priceTo, lengthFilter])
